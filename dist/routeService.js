...
function nivelTrafico() {
    const hora = (new Date().getUTCHours() - 6 + 24) % 24;
    if ((hora >= 7 && hora < 9) || (hora >= 18 && hora < 21))
        return "alto";
    if ((hora >= 9 && hora < 18))
        return "moderado";
    return "bajo";
}
export async function calcularTiempos(userLat, userLng, destinos) {
    // Solo destinos con coordenadas conocidas
    const indexConCoords = [];
    // Primera coordenada = usuario (fuente), resto = destinos
    const pares = [`${userLng},${userLat}`];
    ...
    try {
        ...
        const resultados = destinos.map(() => null);
        ...
        console.error("[routeService] Error al llamar OSRM:", err.message);
        return destinos.map(() => null);
    }
}
