import { RecomendacionSchema } from "./schema.js";
...
async function llamarMlEngine(parametros) {
    ...
    try {
        ...
        throw new MlEngineError(`No se pudo conectar con el motor ML (Capa 2) en ${ML_ENGINE_URL}: ${err.message}`);
    }
    finally {
        clearTimeout(timeoutId);
    }
}
export async function warmupMlEngine() {
    try {
        const controller = new AbortController();
...
export async function obtenerRecomendacion(parametros) {
    ...
    for (let intento = 1; intento <= REINTENTOS_MAX; intento++) {
        ...
        try {
            ...
            if (intento < REINTENTOS_MAX) {
                console.log(`[mlEngineClient] Error en intento ${intento}/${REINTENTOS_MAX}, reintentando en ${ESPERA_ENTRE_REINTENTOS_MS / 1000}s...`);
        ...
        if (response.status === 502 || response.status === 503) {
            ultimoError = new MlEngineError(`El motor ML respondio ${response.status}: el servicio no esta disponible`);
            if (intento < REINTENTOS_MAX) {
                console.log(`[mlEngineClient] Motor ML respondio ${response.status}, reintento ${intento}/${REINTENTOS_MAX} en ${ESPERA_ENTRE_REINTENTOS_MS / 1000}s...`);
                await esperar(ESPERA_ENTRE_REINTENTOS_MS);
                continue;
            }
            break;
