...
import { CATEGORIAS_INTERES, ParametrosViajeSchema } from "./schema.js";
...
function normalizarInteres(raw) {
    if (raw === null || raw === undefined)
        return null;
    const val = String(raw).toLowerCase().trim();
    if (CATEGORIAS_INTERES.includes(val))
        return val;
...
export async function extraerParametros(texto, historial = []) {
    ...
    if (!result.success) {
        ...
        throw new ExtractionError(`La respuesta del modelo no cumple el esquema esperado: ${result.error.message}`);
    }
    return result.data;
}
...
export async function responderConversacional(texto) {
    const completion = await groq.chat.completions.create({
        model: MODEL,
        temperature: 0.7,
        messages: [
